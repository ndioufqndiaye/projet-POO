...
if (isset($_POST['inscrip'])) // Si on a voulu créer un personnage.
{
  ...
  $home= new Chambre($_POST['nom_chambre'],$_POST['id_bat']); // On crée un nouveau personnage.

  $etudiantservice->chambre($home);
...
<div id="form1">
<form id="form" action="form_chambre.php" method="POST">

<div class="form-group row">
    <label for="inputEmail3" id="email" class="col-sm-2 col-form-label">nom_chambre</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="inputEmail3" name="nom_chambre" placeholder="">
    </div>
  </div>
  <div class="form-group row">
  <label for="inputEmail3" id="email" class="col-sm-2 col-form-label">id_batiment</label>
   

            <?php
            echo "<select id='d1' name='id_bat'>" ;
            $batiment=new EtudiantService();
            foreach($batiment->select('batiment') as $key=>$bat){
              echo "<option value=".$bat['id_bat'].">".$bat['nom_bat']."</option>";
            }
            echo "</select>" ;
